    ...
    <div className="flex min-h-screen flex-col bg-black text-white">
      ...
      <section className="relative flex h-[85vh] items-center justify-center overflow-hidden bg-[radial-gradient(ellipse_at_center,var(--tw-gradient-stops))] from-zinc-900 via-black to-black px-6">
        ...
        <div className="relative z-10 mx-auto max-w-5xl text-center flex flex-col items-center">
          ...
          <h1 className="mt-6 font-heading text-4xl font-light tracking-wide text-white sm:text-6xl md:text-7xl leading-tight">
            L'Élégance Olfactive <br />
            <span className="font-serif italic text-gold font-normal">Revisité</span>
          </h1>
          <p className="mt-6 max-w-lg text-sm text-zinc-400 sm:text-base leading-relaxed tracking-wide">
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4">
            <Link
              href="/produits"
              className="inline-flex h-12 items-center justify-center rounded-full bg-gold px-8 text-sm font-semibold uppercase tracking-wider text-black transition-all duration-300 hover:bg-gold-light hover:scale-105 active:scale-95"
            >
              Découvrir la collection
            </Link>
            <Link
              href="/produits?gender=UNISEXE"
              className="inline-flex h-12 items-center justify-center rounded-full border border-zinc-800 bg-zinc-950 px-8 text-sm font-semibold uppercase tracking-wider text-zinc-300 transition-all duration-300 hover:border-gold hover:text-white hover:bg-zinc-900"
            >
              Collection Unisexe
            </Link>
          </div>
      ...
      <section className="py-24">
        <div className="mx-auto max-w-7xl px-6 sm:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <div>
              <span className="text-xs font-semibold uppercase tracking-wider text-gold">Notre Sélection</span>
              <h2 className="mt-2 font-heading text-3xl font-light text-white sm:text-4xl">Les Plus Demandés</h2>
            </div>
            <Link
              href="/produits"
              className="group mt-4 md:mt-0 flex items-center gap-2 text-sm font-semibold text-gold hover:text-gold-light transition-colors"
            >
          ...
          {featuredProducts.length > 0 ? (
            ...
            <div className="text-center py-12 rounded-xl border border-dashed border-zinc-800 bg-zinc-950/20">
              <p className="text-zinc-500 text-sm">Aucun produit vedette disponible pour le moment.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="py-24 border-t border-white/5 bg-zinc-950/20">
        <div className="mx-auto max-w-7xl px-6 sm:px-8">
          <h2 className="text-center font-heading text-2xl font-light text-white tracking-widest uppercase mb-16">
            Nos Grandes Marques
          </h2>
          <div className="grid grid-cols-2 gap-8 md:grid-cols-4 items-center justify-items-center opacity-60">
            {["Chanel", "Dior", "Yves Saint Laurent", "Tom Ford"].map((brand) => (
              <Link
                ...
                className="font-heading text-lg font-bold tracking-widest text-zinc-400 hover:text-gold transition-colors duration-300"
              >
                {brand.toUpperCase()}
              </Link>
            ))}
